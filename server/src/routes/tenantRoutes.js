const express = require('express');
const { 
  updateTenant, 
  getAllLandlordTenants, 
  createTenant, 
  assignTenantsBulk, 
  getMyPropertyAndUnit 
} = require('../controllers/tenantController');
const { requireAuth, requireRole } = require('../middleware/authMiddleware');
const uploadLeaseDoc = require('../middleware/uploadLeaseDoc');

const router = express.Router();

// GET /api/tenants - get all tenants for current landlord
router.get('/', requireAuth, requireRole(['landlord', 'super_admin', 'caretaker']), getAllLandlordTenants);

// POST /api/tenants - create tenant for current landlord
router.post('/', requireAuth, requireRole(['landlord', 'super_admin', 'caretaker']), uploadLeaseDoc.single('leaseDocument'), createTenant);

// POST /api/tenants/assign-bulk - bulk assign tenants to property/unit
router.post('/assign-bulk', requireAuth, requireRole(['landlord', 'super_admin']), assignTenantsBulk);

// PATCH /api/tenants/:id - update tenant details
router.patch('/:id', requireAuth, requireRole(['landlord', 'super_admin']), updateTenant);

// GET /api/tenants/me/property - Tenant fetches their assigned property/unit
router.get('/me/property', requireAuth, requireRole(['tenant', 'landlord', 'super_admin', 'caretaker']), getMyPropertyAndUnit);

module.exports = router;
