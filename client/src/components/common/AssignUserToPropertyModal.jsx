...
const AssignUserToPropertyModal = ({
  open,
  onClose,
  onSubmit,
  mode,
      ...
      if (needsUnitType) {
        ...
        if (unit) {
          const tenantsOfType = Array.isArray(selectedProperty.tenants)
            ? selectedProperty.tenants.filter((t) => t.unitType && t.unitType.toLowerCase() === unitType.toLowerCase())
            : [];
        ...
        if (unit) {
          ...
          if (Array.isArray(selectedProperty.tenants) && selectedProperty.tenants.length > 0 && typeof selectedProperty.tenants[0] === 'object' && selectedProperty.tenants[0].unitType) {
            tenantsCount = selectedProperty.tenants.filter((t) => !t.unitType || t.unitType === unit.type).length;
          } else {
