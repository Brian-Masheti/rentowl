...
interface FinancialReportData {
  ...
  perProperty: PropertyReport[];
}

interface FinancialReportProps {
  type: 'report' | 'monthly-income' | 'arrears' | 'occupancy';
}
...
const FinancialReport: React.FC<FinancialReportProps> = ({ type }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    let fetchFn: () => Promise<FinancialReportData | null>;
    switch (type) {
      case 'report':
  ...
  if (type === 'report' && data && data.summary && data.perProperty) {
    return (
      <div className="bg-white rounded-lg shadow p-6 min-h-[200px]">
        ...
        {/* Per Property Table or Cards */}
        {/* Export/Print/Download Button Group */}
        ...
        <div className="bg-white/90 border border-[#FFA673]/20 rounded-2xl shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 p-6 mt-8">
          <ResponsiveTableOrCards
            columns={[
              ...
              { key: 'expectedRent', label: 'Expected Rent', render: (p: PropertyReport) => formatCurrency(p.expectedRent) },
              { key: 'receivedRent', label: 'Received', render: (p: PropertyReport) => formatCurrency(p.receivedRent) },
              { key: 'outstandingRent', label: 'Outstanding', render: (p: PropertyReport) => formatCurrency(p.outstandingRent) },
              {
                key: 'unitSummary',
                label: 'Units',
                render: (p: PropertyReport) => (
            ...
            keyField="propertyId"
            cardTitle={(p: PropertyReport) => p.propertyName}
            cardContent={(p: PropertyReport) => (
  ...
  // Default fallback for other types or no data
  return (
    <div className="bg-white rounded-lg shadow p-6 min-h-[200px]">
      <h2 className="text-xl font-bold mb-4 text-[#03A6A1]">{TITLES[type]}</h2>
      {!loading && !error && data && (
