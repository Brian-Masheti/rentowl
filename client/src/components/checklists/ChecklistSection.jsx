              ...
              {items.map((item, idx) => (
                <tr key={item.key} className="hover:bg-[#FFE3BB]/60">
                  {columns.map(col => {
                    switch (col.key) {
                      case 'item':
                        return <td key={col.key} className="p-2 font-semibold text-[#03A6A1]">{item.label}</td>;
                      case 'status':
                        return (
                      ...
                      case 'tenant':
                        return <td key={col.key} className="p-2">{tenant?.firstName || ''} {tenant?.lastName || ''}</td>;
                      case 'caretaker':
                        return <td key={col.key} className="p-2">{caretaker?.firstName || ''} {caretaker?.lastName || ''}</td>;
                      case 'property':
                        return <td key={col.key} className="p-2">{property?.name || ''}</td>;
                      case 'unitType':
                        return <td key={col.key} className="p-2">{unitType || ''}</td>;
                      case 'photo':
                        return (
                          <td key={col.key} className="p-2">
                            <CheckListPhotoInput
