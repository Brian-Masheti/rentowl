...
const PropertyEditForm = ({ property, onSuccess, onClose }) => {
  ...
  const [profilePicPreview, setProfilePicPreview] = useState(property.profilePic ? property.profilePic : null);
  const [galleryPreviews, setGalleryPreviews] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [success, setSuccess] = useState(false);
  const initialIsUniform = property.units && property.units.length === 1 ? 'yes' : 'no';
  const [isUniform, setIsUniform] = useState(initialIsUniform);
      ...
      <div className="flex flex-col gap-4 mb-4">
        ...
        <div>
          <div className="flex flex-wrap gap-2 mb-2">
            {property.gallery && property.gallery.length > 0 && property.gallery.map((img, idx) => (
