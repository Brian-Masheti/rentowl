...
const PropertySummary: React.FC<PropertySummaryProps> = ({ name, status, units, tenants }) => {
  // Per-unit-type summary
  const unitSummaries = (units || []).map((unit) => {
    // For each unit type, count tenants assigned to that type
    let occupied = 0;
    if (Array.isArray(tenants) && tenants.length > 0) {
      // If tenants are objects with unitType, count by unitType
      if (typeof tenants[0] === 'object' && tenants[0].unitType) {
        occupied = tenants.filter((t: any) => t.unitType && t.unitType.toLowerCase() === unit.type.toLowerCase()).length;
      } else {
        // Fallback: treat all tenants as occupying any unit (for uniform or legacy data)
