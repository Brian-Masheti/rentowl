...
interface UnassignedTenantSelectorProps {
  onAssign: (tenantId: string, propertyId: string, unitType: string) => void;
  properties: any[];
}

const UnassignedTenantSelector: React.FC<UnassignedTenantSelectorProps> = ({ onAssign, properties }) => {
  ...
  useEffect(() => {
    const fetchUnassigned = async () => {
      ...
      const assignedIds = properties.flatMap((p: any) => (p.tenants || []).map((t: any) => t.tenant?._id || t.tenant));
      setUnassignedTenants(all.filter((t: any) => !assignedIds.includes(t._id)));
      setLoading(false);
    };
    fetchUnassigned();
  }, [properties]);
  ...
  useEffect(() => {
    if (selectedProperty) {
      ...
      setUnitTypes(prop?.units?.map((u: any) => u.type) || []);
      setSelectedUnitType('');
    } else {
      setUnitTypes([]);
      setSelectedUnitType('');
